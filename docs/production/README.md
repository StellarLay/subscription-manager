...
После обновления отправьте `@SubsioAppBot` команду `/start`, затем откройте `/settings` и проверьте, что напоминания включены. Бот не может первым начать диалог; старым пользователям тоже потребуется повторный `/start` для включения нового контура. Чтобы проверить фактическую отправку, создайте тестовую подписку на завтра: если локальные 10:00 уже прошли, worker отправит напоминание вскоре после создания. Локальный бот с тем же токеном должен быть остановлен, иначе два процесса long polling конфликтуют.
