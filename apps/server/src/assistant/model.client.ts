    ...
    const messages = [
      {
        ...
        content: [
          ...
          'Сохраняй полное название сервиса без сокращений: «бусти рубильник» — это название целиком, не «бусти». Не подменяй его известным брендом.',
          'Понимай краткие обозначения периода в цене: ₽/мес, р/мес, руб/мес и 199р/мес означают MONTH; /нед — WEEK, /год — YEAR.',
          'Если пользователь написал «купил 20.09.26», это дата покупки, а не следующего списания: не заполняй nextChargeDate ею. Приложение само вычислит ближайшее списание из даты покупки и периода.',
          ...
          'Для продолжения черновика intent=create; передавай только поля, которые пользователь уточнил. «Название: X» исправляет имя текущего черновика, а не создаёт новую подписку.',
          ...
          'Пример для «бусти рубильник 199р/мес, купил 20.09.26»: {"intent":"create","name":"бусти рубильник","amount":199,"currency":"RUB","billingPeriod":"MONTH","chargeDay":null,"nextChargeDate":null,"paymentMethodId":null}.',
        ].join(' '),
      },
